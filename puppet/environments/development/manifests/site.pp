...
node 'clientlinux.mshome.net' {

  exec { 'run_my_script':
      command => '/vagrant/software/h2/bin/camunda.sh',
      refreshonly => true
  }
  ...
  include mt
  include pack_domain
  include start_managed_instances
  include start_domain_partitions_instances
  include deployments

  # # include ora_em_agent

  Class[java] -> Class[orawls::weblogic] -> Class[start_managed_instances] -> Class[start_domain_partitions_instances]
...
class os {
  ...
  $node1_address = hiera('node1_address')
  file { '/etc/sysconfig/network-scripts/ifcfg-eth0:0':
    notify  => Service['network'],
    mode    => '0600',
    owner   => 'root',
    group   => 'root',
    content => "DEVICE=eth0:0
IPADDR=$node1_address
NETMASK=255.0.0.0
NETWORK=123.0.0.0
...
ONBOOT=yes",
  ...
  $node2_address = hiera('node2_address')
...
class start_managed_instances {
  require domains, nodemanager

  $default_params = {}
  $control_managed_instances = hiera('control_managed_instances', {})
  create_resources('orawls::control',$control_managed_instances, $default_params)
}
...
class basic_config{
  ...
  $cluster_instances = hiera('multi_server_action') ? {
    'start' => hiera('cluster_instances_multi_server', {}),
    'stop'  => hiera('cluster_instances_single_server', {}),
    default => fail("valor incorreto para multi_server_action")
  }
...
class start_domain_partitions_instances {
  $default_params = {}
  $control_domain_partitions_instances = hiera('control_domain_partitions_instances', {})
  create_resources('wls_domain_partition_control',$control_domain_partitions_instances, $default_params)
}
